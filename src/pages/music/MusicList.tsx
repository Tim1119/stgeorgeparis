...
import { musicListEntries } from '../../data/musicList';
...
export default function MusicList() {
  ...
  const filtered = useMemo(() => {
    return musicListEntries.filter((e) => {
      if (dateFrom && e.date < dateFrom) return false;
      if (dateTo && e.date > dateTo) return false;
      if (search.trim()) {
        const q = search.trim().toLowerCase();
        const haystack = `${e.setting} ${e.anthem} ${e.hymns}`.toLowerCase();
      ...
      <div className="max-w-5xl mx-auto px-4 py-16">
        <div className="bg-gray-50 border border-gray-100 rounded-xl p-6 mb-10 grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="sm:col-span-1">
            <label className="block text-xs font-bold text-gray-500 uppercase mb-1.5">Search setting or anthem</label>
            <div className="relative">
              ...
              <input
                ...
                placeholder="e.g. Byrd, Palestrina…"
        ...
        <p className="text-sm text-gray-500 mb-4">{filtered.length} entries found</p>

        <div className="divide-y divide-gray-100 border-t border-b border-gray-100">
          {pageItems.map((e) => (
            <div key={e.id} className="py-5 flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6">
              ...
              <div className="flex-grow">
                <p className="font-bold text-gray-900">{e.setting}</p>
                <p className="text-sm text-gray-500">
                  Anthem: <span className="text-primary">{e.anthem}</span> &middot; Hymns: {e.hymns}
                </p>
              </div>
          ...
          {pageItems.length === 0 && (
            <p className="py-12 text-center text-gray-400">No entries match those filters.</p>
          )}
        </div>
